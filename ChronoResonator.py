...
class ChronoResonator:
    def __init__(self, use_gui=True):
        ...
        if self.use_gui:
            ...
            self.flux_art = tk.Label(self.root, text="""
                _.-._
               / \_/ \
              /_______\
              |  ***  | 
    ...
    def run_resonator(self):
        try:
            ...
            coherence = abs(math.sin(2 * math.pi * self.frequency * duration / self.phi) * math.exp(-duration / self.tau))
            ...
            success_msg = f"Flux Capacitor Activated! Time jump to {target_time}!\nGoldenGravity-powered, Manuela!"
            if self.use_gui:
